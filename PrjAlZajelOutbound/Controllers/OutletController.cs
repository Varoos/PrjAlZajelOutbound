    ...
    public class OutletController : Controller
    {
        ...
        public ActionResult MasterPosting(int CompanyId, string SessionId, string Name, string Code)
        {
            ...
            try
            {
                ...
                if (AccessToken == "")
                {
                    ...
                    objreg.SetLog("AlZajelOutlet.log", "Invalid Token");
                    //FConvert.LogFile("AlZajelOutlet.log", DateTime.Now.ToString() + " : Invalid Token");
                    var sMessage = "Token Should not be Empty";
                    objreg.SetLog("AlZajelOutlet.log", sMessage);
                    //FConvert.LogFile("AlZajelOutlet.log", DateTime.Now.ToString() + " : " + sMessage);
                ...
                if (isGroup == 0)
                {
                    ...
                    using (WebClient client = new WebClient())
                    {
                        ...
                        objreg.SetLog("AlZajelOutlet.log", " PostingURL :" + PostingURL);
                        ...
                        if (lng.ResponseStatus.IsSuccess == true)
                        {
                            ...
                            {
                                string UpSql = $@"update muPos_Outlet set Posted=1,PostedDate={strdate} where iMasterId={clist.ItemList.First().FldId}";
                                res = objreg.GetExecute(UpSql, CompanyId, ref errors1);
                            }
                            else
                            {
                                string UpSql = $@"update muPos_Outlet set Posted=2,PostedDate={strdate} where iMasterId={clist.ItemList.First().FldId}";
                                res = objreg.GetExecute(UpSql, CompanyId, ref errors1);
                            }
                            if (res == 1)
                            {
                                objreg.SetLog("AlZajelOutlet.log", "Data posted / updated to mobile app device");
                                //FConvert.LogFile("AlZajelOutlet.log", DateTime.Now.ToString() + "Data posted / updated to mobile app device");
                            }
                        ...
                        else
                        {
                            ...
                            {
                                objreg.SetLog("AlZajelOutlet.log", "Error Message for Outlet Master:" + item);
                                //FConvert.LogFile("AlZajelOutlet.log", DateTime.Now.ToString() + "Error Message for Outlet Master:" + item);

                                string UpSql = $@"update muPos_Outlet set Posted=2 where iMasterId={clist.ItemList.First().FldId}";
                            ...
                            if (FailedListCount > 0)
                            {
                                ...
                                {
                                    objreg.SetLog("AlZajelOutlet.log", "FailedList Message for Outlet Master with Master Id:" + item.FldId + " and Code:" + item.FldCode);
                                    //FConvert.LogFile("AlZajelOutlet.log", DateTime.Now.ToString() + "FailedList Message for Outlet Master with Master Id:" + item.FldId + " and Code:" + item.FldCode);
            ...
            catch (Exception e)
            {
                errors1 = e.Message;
                Message = e.Message;
                objreg.SetLog("AlZajelOutlet.log", " Error :" + errors1);
                //FConvert.LogFile("AlZajelOutlet.log", DateTime.Now.ToString() + " Error :" + errors1);
            }
            return Json(new { Message = Message }, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult MasterDelete(int companyId, string Name, string Code)
        {
            ...
            try
            {
                ...
                string sql = $@"select o.iMasterId,sName,sCode,mo.CompanyMaster [OutletId],case when OutletTypeID=0 then 1 else 2 end OutletTypeID,PlateNumber,convert(varchar,dbo.IntToDate(RegistrationDate), 23) RegistrationDate,
                                Employee,iCreatedDate,iModifiedDate from mpos_Outlet o
                                join muPos_Outlet mo on mo.iMasterId=o.iMasterId
                ...
                {
                    client.Headers.Add("Content-Type", "application/json");
                    objreg.SetLog("AlZajelOutlet.log", "Delete PostingURL :" + PostingURL);
                    var arrResponse = client.UploadString(PostingURL, sContent);
                    ...
                    if (lng.ResponseStatus.IsSuccess == true)
                    {
                        ...
                        objreg.SetLog("AlZajelOutlet.log", "Delete Response" + Message);
                        //FConvert.LogFile("AlZajelOutlet.log", DateTime.Now.ToString() + "Delete Response" + Message);
                    }
                    else
                    {
                        objreg.SetLog("AlZajelOutlet.log", "Delete Failed Response" + lng.ResponseStatus.StatusMsg);
                        //FConvert.LogFile("AlZajelOutlet.log", DateTime.Now.ToString() + "Delete Failed Response" + lng.ResponseStatus.StatusMsg);
                        ...
                        foreach (var item in ErrorMessagesList)
                        {
                            Message = item;
                            objreg.SetLog("AlZajelOutlet.log", "Error Message for Outlet Master:" + item);
                            //FConvert.LogFile("AlZajelOutlet.log", DateTime.Now.ToString() + "Error Message for Employee Master:" + item);
                        }
        ...
        public string GetAccessToken()
        {
            ...
            {
                client.Headers.Add("Content-Type", "application/json");
                objreg.SetLog("AlZajelOutlet.log", " AccessTokenURL :" + AccessTokenURL);
