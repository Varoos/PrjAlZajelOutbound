...
function LoadItemMaster(obj, a, b) {
    debugger
...
function GetSetActMasterFields() {
    try {
        ...
        Focus8WAPI.getFieldValue("fnGetValueCallBackItem", ["sName", "sCode"], 1, false, 111);
    }
    catch (err) {
...
function fnGetValueCallBackItem(objWrapperResult) {
    try {
        debugger
        ...
        if (responseData != null && responseData.length > 0) {
            if (objWrapperResult.requestType == 1) {
                if (objWrapperResult.iRequestId == 111) {
                    ...
                    $.ajax({
                        url: baseUrl + "/Item/MasterPosting",
                        cache: false,
                        type: "POST",
                        datatype: 'JSON',
...
function BeforeDelete() {
    debugger
    Focus8WAPI.getFieldValue("ItemDataCallbackBeforeDelete", ["sName", "sCode", "iMasterId"], Focus8WAPI.ENUMS.MODULE_TYPE.MASTER, false, 111);
}

function ItemDataCallbackBeforeDelete(response1) {
    debugger;
    try {
        debugger;
