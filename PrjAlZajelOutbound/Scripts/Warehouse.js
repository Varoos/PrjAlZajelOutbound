var baseUrl = 'http://localhost/PrjAlZajelOutbound';
var loginDetails = {};
var CompId = 0;
var SessionId = "";
var docNo = "0";
...
function LoadWarehouseMaster(obj, a, b) {
    debugger
...
function GetSetActMasterFields() {
    try {
        ...
        Focus8WAPI.getFieldValue("fnGetValueCallBackWarehouse", ["sName", "sCode"], 1, false, 111);
    }
    catch (err) {
...
function fnGetValueCallBackWarehouse(objWrapperResult) {
    try {
        debugger
        ...
        if (responseData != null && responseData.length > 0) {
            if (objWrapperResult.requestType == 1) {
                if (objWrapperResult.iRequestId == 111) {
                    ...
                    $.ajax({
                        url: baseUrl + "/Warehouse/MasterPosting",
                        cache: false,
                        type: "POST",
                        datatype: 'JSON',
...
function BeforeDelete() {
    debugger
    Focus8WAPI.getFieldValue("WarehouseDataCallbackBeforeDelete", ["sName", "sCode", "iMasterId"], Focus8WAPI.ENUMS.MODULE_TYPE.MASTER, false, 111);
}

function WarehouseDataCallbackBeforeDelete(response1) {
    debugger;
    try {
        debugger;
...
function WarehouseDelete() {
    debugger;
    Focus8WAPI.getGlobalValue("WarehouseCallbackDelete", '*', 1);
}

function WarehouseCallbackDelete(response) {
    debugger;
    ...
    $.ajax({
        url: baseUrl + "/Warehouse/MasterDelete",
        type: "POST",
