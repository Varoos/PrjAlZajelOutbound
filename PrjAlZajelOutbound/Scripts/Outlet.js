...
function LoadOutletMaster(obj, a, b) {
    debugger
...
function GetSetActMasterFields() {
    try {
        ...
        Focus8WAPI.getFieldValue("fnGetValueCallBackOutlet", ["sName", "sCode"], 1, false, 111);
    }
    catch (err) {
...
function fnGetValueCallBackOutlet(objWrapperResult) {
    try {
        debugger
        ...
        if (responseData != null && responseData.length > 0) {
            if (objWrapperResult.requestType == 1) {
                if (objWrapperResult.iRequestId == 111) {
                    ...
                    $.ajax({
                        url: baseUrl + "/Outlet/MasterPosting",
                        cache: false,
                        type: "POST",
                        datatype: 'JSON',
...
function BeforeDelete() {
    debugger
    Focus8WAPI.getFieldValue("OutletDataCallbackBeforeDelete", ["sName", "sCode", "iMasterId"], Focus8WAPI.ENUMS.MODULE_TYPE.MASTER, false, 111);
}

function OutletDataCallbackBeforeDelete(response1) {
    debugger;
    try {
        debugger;
...
function OutletDelete() {
    debugger;
    Focus8WAPI.getGlobalValue("OutletCallbackDelete", '*', 1);
}

function OutletCallbackDelete(response) {
    debugger;
    ...
    $.ajax({
        url: baseUrl + "/Outlet/MasterDelete",
        type: "POST",
