...
function LoadBranchMaster(obj, a, b) {
    debugger
...
function GetSetActMasterFields() {
    try {
        ...
        Focus8WAPI.getFieldValue("fnGetValueCallBackBranch", ["sName", "sCode"], 1, false, 111);
    }
    catch (err) {
...
function fnGetValueCallBackBranch(objWrapperResult) {
    try {
        debugger
        ...
        if (responseData != null && responseData.length > 0) {
            if (objWrapperResult.requestType == 1) {
                if (objWrapperResult.iRequestId == 111) {
                    ...
                    $.ajax({
                        url: baseUrl + "/Branch/MasterPosting",
                        cache: false,
                        type: "POST",
                        datatype: 'JSON',
...
function BeforeDelete() {
    debugger
    Focus8WAPI.getFieldValue("BranchDataCallbackBeforeDelete", ["sName", "sCode", "iMasterId"], Focus8WAPI.ENUMS.MODULE_TYPE.MASTER, false, 111);
}

function BranchDataCallbackBeforeDelete(response1) {
    debugger;
    try {
        debugger;
...
function BranchDelete() {
    debugger;
    Focus8WAPI.getGlobalValue("BranchCallbackDelete", '*', 1);
}

function BranchCallbackDelete(response) {
    debugger;
    ...
    $.ajax({
        url: baseUrl + "/Branch/MasterDelete",
        type: "POST",
