...
function LoadEmployeeMaster(obj, a, b) {
    debugger
...
function GetSetActMasterFields() {
    try {
        ...
        Focus8WAPI.getFieldValue("fnGetValueCallBackEmployee", ["sName", "sCode"], 1, false, 111);
    }
    catch (err) {
...
function fnGetValueCallBackEmployee(objWrapperResult) {
    try {
        debugger
        ...
        if (responseData != null && responseData.length > 0) {
            if (objWrapperResult.requestType == 1) {
                if (objWrapperResult.iRequestId == 111) {
                    ...
                    $.ajax({
                        url: baseUrl + "/Employee/MasterPosting",
                        cache: false,
                        type: "POST",
                        datatype: 'JSON',
...
function BeforeDelete() {
    debugger
    Focus8WAPI.getFieldValue("EmployeeDataCallbackBeforeDelete", ["sName", "sCode", "iMasterId"], Focus8WAPI.ENUMS.MODULE_TYPE.MASTER, false, 111);
}

function EmployeeDataCallbackBeforeDelete(response1) {
    debugger;
    try {
        debugger;
...
function EmployeeDelete() {
    debugger;
    Focus8WAPI.getGlobalValue("EmployeeCallbackDelete", '*', 1);
}

function EmployeeCallbackDelete(response) {
    debugger;
    ...
    $.ajax({
        url: baseUrl + "/Employee/MasterDelete",
        type: "POST",
