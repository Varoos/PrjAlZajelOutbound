    ...
    public class BranchController : Controller
    {
        ...
        public ActionResult MasterPosting(int CompanyId, string SessionId, string Name, string Code)
        {
            ...
            try
            {
                ...
                if (AccessToken == "")
                {
                    ...
                    objreg.SetLog("AlZajelBranch.log", "Invalid Token");
                    //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + " : Invalid Token");
                    var sMessage = "Token Should not be Empty";
                    objreg.SetLog("AlZajelBranch.log", sMessage);
                    //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + " : " + sMessage);
                ...
                if (isGroup == 0)
                {
                    ...
                    using (WebClient client = new WebClient())
                    {
                        ...
                        objreg.SetLog("AlZajelBranch.log", " PostingURL :" + PostingURL);
                        ...
                        if (lng.ResponseStatus.IsSuccess == true)
                        {
                            ...
                            {
                                string UpSql = $@"update muCore_CompanyMaster set Posted=1,PostedDate={strdate} where iMasterId={clist.ItemList.First().FldId}";
                                res = objreg.GetExecute(UpSql, CompanyId, ref errors1);
                            }
                            else
                            {
                                string UpSql = $@"update muCore_CompanyMaster set Posted=2,PostedDate={strdate} where iMasterId={clist.ItemList.First().FldId}";
                                res = objreg.GetExecute(UpSql, CompanyId, ref errors1);
                            }
                            if (res == 1)
                            {
                                objreg.SetLog("AlZajelBranch.log", "Data posted / updated to mobile app device");
                                //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + "Data posted / updated to mobile app device");
                            }
                        ...
                        else
                        {
                            ...
                            {
                                objreg.SetLog("AlZajelBranch.log", "Error Message for Branch Master:" + item);
                                //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + "Error Message for Branch Master:" + item);
                            ...
                            if (FailedListCount > 0)
                            {
                                ...
                                {
                                    objreg.SetLog("AlZajelBranch.log", "FailedList Message for Branch Master with Master Id:" + item.FldId + " and Code:" + item.FldCode);
                                    //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + "FailedList Message for Branch Master with Master Id:" + item.FldId + " and Code:" + item.FldCode);
            ...
            catch (Exception e)
            {
                errors1 = e.Message;
                Message = e.Message;
                objreg.SetLog("AlZajelBranch.log", " Error :" + errors1);
                //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + " Error :" + errors1);
            }
            return Json(new { Message = Message }, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult MasterDelete(int companyId, string Name, string Code)
        {
            ...
            try
            {
                ...
                {
                    client.Headers.Add("Content-Type", "application/json");
                    objreg.SetLog("AlZajelBranch.log", "Delete PostingURL :" + PostingURL);
                    var arrResponse = client.UploadString(PostingURL, sContent);
                    ...
                    if (lng.ResponseStatus.IsSuccess == true)
                    {
                        ...
                        objreg.SetLog("AlZajelBranch.log", "Delete Response" + Message);
                        //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + "Delete Response" + Message);
                    }
                    else
                    {
                        objreg.SetLog("AlZajelBranch.log", "Delete Failed Response" + lng.ResponseStatus.StatusMsg);
                        //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + "Delete Failed Response" + lng.ResponseStatus.StatusMsg);

                        var ErrorMessagesList = lng.ErrorMessages.ToList();
                        foreach (var item in ErrorMessagesList)
                        {
                            objreg.SetLog("AlZajelBranch.log", "Error Message for Employee Master:" + item);
                            //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + "Error Message for Employee Master:" + item);
                            Message = item;
                        }
                        ...
                        if (FailedListCount > 0)
                        {
                            ...
                            foreach (var item in FailedList)
                            {
                                objreg.SetLog("AlZajelBranch.log", "FailedList Message for Employee Master with Master Id:" + item.FldId + " and Code:" + item.FldCode);
                                //FConvert.LogFile("AlZajelBranch.log", DateTime.Now.ToString() + "FailedList Message for Employee Master with Master Id:" + item.FldId + " and Code:" + item.FldCode);
                                var FieldMasterId = item.FldId;
                            }
                        }
                    }
        ...
        public string GetAccessToken()
        {
            ...
            {
                client.Headers.Add("Content-Type", "application/json");
                objreg.SetLog("AlZajelBranch.log", " AccessTokenURL :" + AccessTokenURL);
